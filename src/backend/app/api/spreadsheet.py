...
class FilterCriteria(BaseModel):
    column: str
    condition: str = Field(..., pattern="^(equals|notEquals|greaterThan|lessThan|greaterThanOrEqual|lessThanOrEqual|contains|notContains|startsWith|endsWith|empty|notEmpty|between|in)$")
    value: Any
...
class FormulaEngine:
    ...
    def _parse_and_eval(self, expr: str) -> Any:
        ...
        if expr.replace(':', '').replace('$', '').isalnum() and any(c.isalpha() for c in expr):
    ...
    def _call_function(self, name: str, args: List[Any]) -> Any:
        ...
        functions = {
            ...
            'AVERAGE': lambda a: sum(self._flatten(a)) / len(self._flatten(a)) if self._flatten(a) else 0,
            'COUNT': lambda a: len([x for x in self._flatten(a) if isinstance(x, (int, float))]),
...
async def sync_to_cloud(request: CloudSyncRequest):
    ...
    try:
        # This would implement the actual cloud sync logic
